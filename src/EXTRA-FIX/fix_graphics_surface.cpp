...
namespace {
constexpr int NGRID = 128;
constexpr double SURFMAX = 4.0;
constexpr double SURFHIGH = 3.0;
constexpr double SURFMED = 2.5;
constexpr double SURFLOW = 2.0;
...
{
  if (narg < 4) utils::missing_cmd_args(FLERR, "fix graphics/surface", error);
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "sphere") == 0) {
      if (iarg + 6 > narg) utils::missing_cmd_args(FLERR, "fix graphics/surface sphere", error);
      // clang-format off
      ...
      if (iarg + 9 > narg) utils::missing_cmd_args(FLERR, "fix graphics/surface cylinder", error);
      // clang-format off
      ...
      if (iarg + 10 > narg) utils::missing_cmd_args(FLERR, "fix graphics/surface arrow", error);
      // clang-format off
      ...
      if (iarg + 11 > narg) utils::missing_cmd_args(FLERR, "fix graphics/surface progbar", error);
      // clang-format off
      ...
      if (iarg + 6 > narg) utils::missing_cmd_args(FLERR, "fix graphics/surface surface", error);
      // clang-format off
      SurfaceItem surface{SURFACE, 1, nullptr, 0.5, 1.5, 2.0, nullptr, -1};
      // clang-format on
      surface.type = utils::inumeric(FLERR, arg[iarg + 1], false, lmp);
      if (group->find(arg[iarg + 2]) < 0) {
        error->all(FLERR, iarg + 2, "Group {} for fix graphics/surface surface does not exist",
                   arg[iarg + 2]);
      } else {
        surface.groupname = utils::strdup(arg[iarg + 2]);
      }
      surface.iso = utils::numeric(FLERR, arg[iarg + 3], false, lmp);
      if (surface.iso <= 0.0) error->all(FLERR, 3, "Illegal surface isovalue {}", surface.iso);
      if (strstr(arg[iarg + 4], "v_") == arg[iarg + 4]) {
        varflag = 1;
        surface.rstr = utils::strdup(arg[iarg + 4] + 2);
      } else {
        surface.rad = utils::numeric(FLERR, arg[iarg + 4], false, lmp);
      }
      if (strstr(arg[iarg + 5], "max") == 0) {
        surface.quality = SURFMAX;
      } else if (strstr(arg[iarg + 5], "high") == 0) {
        surface.quality = SURFHIGH;
      } else if (strstr(arg[iarg + 5], "medium") == 0) {
        surface.quality = SURFMED;
      } else if (strstr(arg[iarg + 5], "low") == 0) {
        surface.quality = SURFLOW;
      } else {
        error->all(FLERR, iarg + 5, "Unknown surface quality setting {}", arg[iarg + 5]);
      }
      items.emplace_back(std::move(surface));
      iarg += 6;
    } else {
  ...
  for (auto &gi : items) {
    switch (gi.style) {
      ...
      case PROGBAR:
        ...
        break;
      case SURFACE:
        delete[] gi.surface.groupname;
        delete[] gi.surface.rstr;
        break;
      default:;    // do nothing
        break;
    }
  ...
  for (auto &gi : items) {
    if (gi.style == SPHERE) {
      ...
      if (gi.sphere.xstr) {
        ...
        if (ivar < 0)
          error->all(FLERR, Error::NOLASTLINE, "Variable name {} for fix graphics/surface does not exist",
                     gi.sphere.xstr);
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.sphere.xstr);
        gi.sphere.xvar = ivar;
      }
      if (gi.sphere.ystr) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.sphere.ystr);
        gi.sphere.yvar = ivar;
      }
      if (gi.sphere.zstr) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.sphere.zstr);
        gi.sphere.zvar = ivar;
      }
      if (gi.sphere.dstr) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.sphere.dstr);
        gi.sphere.dvar = ivar;
      }
      ++n;
      ...
      if (gi.cylinder.x1str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.cylinder.x1str);
      ...
      if (gi.cylinder.y1str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.cylinder.y1str);
      ...
      if (gi.cylinder.z1str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.cylinder.z1str);
      ...
      if (gi.cylinder.x2str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.cylinder.x2str);
      ...
      if (gi.cylinder.y2str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.cylinder.y2str);
      ...
      if (gi.cylinder.z2str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.cylinder.z2str);
      ...
      if (gi.cylinder.dstr) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.cylinder.dstr);
      ...
      if (gi.arrow.x1str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.arrow.x1str);
        gi.arrow.x1var = ivar;
      }
      if (gi.arrow.y1str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.arrow.y1str);
        gi.arrow.y1var = ivar;
      }
      if (gi.arrow.z1str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.arrow.z1str);
        gi.arrow.z1var = ivar;
      }
      if (gi.arrow.x2str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.arrow.x2str);
        gi.arrow.x2var = ivar;
      }
      if (gi.arrow.y2str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.arrow.y2str);
        gi.arrow.y2var = ivar;
      }
      if (gi.arrow.z2str) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.arrow.z2str);
        gi.arrow.z2var = ivar;
      }
      if (gi.arrow.dstr) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.arrow.dstr);
        gi.arrow.dvar = ivar;
      }
      ++n;
      imgobjs[n] = DumpImage::CYLINDER;
      ...
      if (gi.progbar.pstr) {
        ...
        if (input->variable->equalstyle(ivar) == 0)
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal-style variable", gi.progbar.pstr);
        gi.progbar.pvar = ivar;
      }
    } else if (gi.style == SURFACE) {
      if (gi.surface.groupname) {
        int surfbit = group->get_bitmask_by_id(FLERR, gi.surface.groupname, "fix graphics/surface surface");
        int surfidx = group->find(gi.surface.groupname);
        const int nlocal = atom->nlocal;
        const int *const mask = atom->mask;
        const double *const *const x = atom->x;
        bigint numatoms = group->count(surfidx);
        double minmax[6];
        group->bounds(surfidx, minmax);
        fprintf(stderr, "Found %d atoms in group %s\n", numatoms, gi.surface.groupname);
        fprintf(stderr, "MIN/MAX: x(%g:%g) y(%g:%g) z(%g:%g)\n", minmax[0], minmax[1], minmax[2],
                minmax[3], minmax[4], minmax[5]);
      }

      if (gi.surface.rstr) {
        int ivar = input->variable->find(gi.surface.rstr);
        if (ivar < 0)
          error->all(FLERR, Error::NOLASTLINE, "Variable name {} for fix graphics/surface does not exist",
                     gi.surface.rstr);
        if ((input->variable->equalstyle(ivar) == 0) && (input->variable->atomstyle(ivar) == 0))
          error->all(FLERR, Error::NOLASTLINE,
                     "fix graphics/surface variable {} is not equal- or atom-style variable",
                     gi.surface.rstr);
        gi.surface.rvar = ivar;
      }
    }
  }
  end_of_step();
